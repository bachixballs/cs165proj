...
//	SSL_CTX_set_options(ctx, SSL_OP_ALL | SSL_OP_NO_SSLv2 | SSL_OP_SINGLE_DH_USE);
    ...
    challenge = buff; 
//cout << "printing buff_len " << buff_len << endl;
	printf("DONE.\n");
	printf("    (Challenge: \"%s\")\n", challenge.c_str());
	...
	//BIO_new(BIO_s_mem());
        bin = BIO_new(BIO_s_mem()); //making a bio to store the sha1
	//BIO_write
	...
	//BIO_push;
	BIO_push(hash, bin); // make a chain of the bios to store the extended hash
	//BIO_gets;
	int mdlen=0;
	mdlen = BIO_gets(hash, mdbuf, EVP_MAX_MD_SIZE);
    ...
    //RSA_private_encrypt
        int siglen=0;
	siglen = RSA_private_encrypt(size-11, (const unsigned char*)mdbuf, buff_dump, rsa, RSA_PKCS1_PADDING);
	...
	if(flush == -1){print_errors();}
	//SSL_write
    int sig_len = 0;
    char sig_buf[BUFFER_SIZE];
    memset(sig_buf,0, sizeof(sig_buf));
    memcpy(sig_buf, signature, sizeof(sig_buf)); //copy the information in the signature string and put it in the sig_buf to send through the ssl 
    sig_len = SSL_write(ssl, sig_buf, BUFFER_SIZE);
    //cout << sig_len << endl; 
   if(sig_len <= 0) {print_errors();} 
    printf("DONE.\n");
    //-------------------------------------------------------------------------
